...
for count in range(len(finished_text)):
    if count == 0:
        with open("test.txt", "w", encoding="utf-8") as f:
            f.write(finished_text[count] + "page" + str(count + 1))
    else:
        with open("test.txt", "a", encoding="utf-8") as f:
            f.write("\n" + finished_text[count] + "\n" + "page" + str(count + 1) + "\n")

#每次完成之后都会生成一堆用过的temp文件，有些会删掉，有些不会，因此加一个删除temp的过程
#这个不太理解imagemagick这个东西的缓存文件的处理逻辑
dellist = os.listdir("F:/Temp")
for i in dellist:
    try:
        if "magick" in i:
            os.remove("F:/Temp/"+i)
